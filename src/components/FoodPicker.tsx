...
const FoodPicker: React.FC<FoodPickerProps> = ({ meal, existingFoodIds = [], onSelect, onClose }) => {
  ...
  const handleFoodTap = (food: Fodmap) => { setSelectedFood(food); setQuantity(3); };
  const handleConfirm = () => { if (!selectedFood) return; onSelect(selectedFood, quantity); setSelectedFood(null); };
  ...
  return (
    <div className="flex flex-col min-h-screen bg-background-secondary">
      ...
      <p className="px-4 py-2 text-xs font-semibold uppercase tracking-wide text-foreground-muted bg-background-tertiary border-b border-divider">
        Adding to {meal}
      </p>
      <div className="overflow-y-auto flex-1 bg-background-primary">
        <InfiniteScroll
          start={0}
        ...
        >
          {currentItems.map((food, index) => (
            <button
              ...
              className="w-full text-left px-4 py-3 border-b border-divider flex items-center justify-between"
            >
              <div>
      ...
      {selectedFood && (
        <div className="fixed inset-0 z-30 flex flex-col justify-end items-center">
          ...
          <div className="relative z-40 bg-background-primary rounded-t-2xl p-5 shadow-xl w-full max-w-screen-sm">
            <div className="flex items-center justify-between mb-5">
              <div>
                <p className="text-lg font-medium text-foreground-primary">{selectedFood.name}</p>
                <p className="text-sm text-foreground-muted">{selectedFood.category}</p>
              </div>
              <Score text={selectedFood.fodmap} score={selectedFood.fodmap === "high" ? 2 : 0} />
            </div>
            <p className="text-xs font-semibold uppercase tracking-wide text-foreground-muted mb-3">Quantity</p>
            <input
              type="range" min={1} max={5} step={1} value={quantity}
              onChange={(e) => setQuantity(Number(e.target.value))}
              className="w-full accent-green-400"
            />
            <div className="flex justify-between text-xs text-foreground-muted mt-1 mb-1">
              {QUANTITY_LABELS.map((label) => <span key={label}>{label}</span>)}
            </div>
            <p className="text-center text-green-600 font-semibold mb-5">{QUANTITY_LABELS[quantity - 1]}</p>
